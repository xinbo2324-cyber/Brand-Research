...
// This stable pair is updated by the administrator upload flow. Historical
// v10/v11/v12 files remain in the repository as recoverable backups.
const input = 'brand-research-summary-live.xlsx';
const output = 'brand-research-summary-live.json.gz';
const trendOutput = 'brand-research-trend-live.json.gz';
const salesOutput = 'brand-research-sales-live.json.gz';
...
const trendSales = trend.map(item => months.map(month => item.sales[month] || 0));
trend.forEach(item => { delete item.sales; });
const payload = {
  version: 'summary-live', sourceName: '管理员上传数据 · 品牌汇总', months, trendMonths: months, trendMetaFile: trendOutput, trendSalesFile: salesOutput,
  latest, qa: { sourceRows: rows.length, dedupedRows: deduped.length, latestParents: latest.length, snapshotMonths, sameTimeConflicts }
};
const json = JSON.stringify(payload);
fs.writeFileSync(output, zlib.gzipSync(json, { level: 9 }));
const trendJson = JSON.stringify({ trendMonths: months, trend });
fs.writeFileSync(trendOutput, zlib.gzipSync(trendJson, { level: 9 }));
const salesJson = JSON.stringify({ months, sales: trendSales });
fs.writeFileSync(salesOutput, zlib.gzipSync(salesJson, { level: 9 }));
console.log(JSON.stringify({ output, jsonBytes: Buffer.byteLength(json), bytes: fs.statSync(output).size, trendOutput, trendJsonBytes: Buffer.byteLength(trendJson), trendBytes: fs.statSync(trendOutput).size, salesOutput, salesJsonBytes: Buffer.byteLength(salesJson), salesBytes: fs.statSync(salesOutput).size, ...payload.qa }));
